...
public class SponsorPayPluginMonoBehaviour : MonoBehaviour
{
	...
	void Awake ()
	{
		if (PluginInstance == null) {
			PluginInstance = new SponsorPay.SponsorPayPlugin (gameObject);

			// Keep the reference to the 1st gameObject we were created with
			DontDestroyOnLoad(gameObject);
		} else if (FindObjectsOfType(GetType()).Length > 1) {
			// If we go to a new scene where this script is present, we destroy its object to avoid having two of them
			// In the dev portal, we say that this object should be an empty one.
			// If we come back to the scene where we were created, we destroy it as well. If this is a meaningful game object,
			// it will stay in the scene because of the original DontDestroyOnLoad
			Destroy(gameObject);
		}
	}
...
namespace SponsorPay
{
	...
	public class SponsorPayPlugin
	{
		...
		public const string PluginVersion = "7.2.11";
		...
		public string Start (string appId, string userId, string securityToken)
		{
			try
			{
				...
				return plugin.StartSDK (appId, userId, securityToken);
			}
			catch (Exception e)
			{
